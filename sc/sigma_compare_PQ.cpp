...
// Modified by Bill Mandel to reduce amount of printout and to add some printout of PQ 12 bit codevalues
// to build:
/* 
...
{
float gMin[3]={65535.0,65535.0, 65535.0},gMax[3]={-65535.0,-65535.0, -65535.0}; // global image sequence min and max
short x, y, c, j, k;
...
/* store difference from original in pixels2 */
          ...
          // check for gMin/gMax
          if(col[c] > 0.0001) {
          gMin[c] = (col[c]<gMin[c]) ? col[c] : gMin[c];
          gMax[c] = (col[c]>gMax[c]) ? col[c] : gMax[c];
		  }
...
printf("\n");
        ...
        if (c==0) { printf(" red - Min: %e Max: %e Range (f-stops): %0.2f\n", gMin[c], gMax[c], log(gMax[c] / gMin[c])/log(2.0)); }
        if (c==1) { printf(" green -  Min: %e Max: %e Range (f-stops): %0.2f\n", gMin[c], gMax[c], log(gMax[c] / gMin[c])/log(2.0)); }
        if (c==2) { printf(" blue - Min: %e Max: %e Range (f-stops): %0.2f\n", gMin[c], gMax[c], log(gMax[c] / gMin[c])/log(2.0)); }
      } /* gMin/gMax */
...
printf("\n");
      ...
      for (j=0; j<NUMBER_OF_STOPS; j++) {
        if (count_col[c][j] > 0) {
          ...
          // Calculate PQ OETF for average
          double averageL = average_col[c][j]/10000.0;
          if (averageL<0.0) {averageL = 0.0;}
          if (averageL>1.0) {averageL = 1.0;}
          int PQ = 256 +(int)(3583.0*PQ10000_r( averageL)+0.5);
          
          // Calculate PQ +/- one sigma range
          double oneSigPQPlus = (sigma_col[c][j] + average_col[c][j])/10000.0;
          if (oneSigPQPlus<0.0) {oneSigPQPlus = 0.0;}
          if (oneSigPQPlus>1.0) {oneSigPQPlus = 1.0;}
          int PQPlus = 256 +(int)(3583.0*PQ10000_r( oneSigPQPlus)+0.5);
          
          double oneSigPQMinus = (-sigma_col[c][j] + average_col[c][j])/10000.0;
          if (oneSigPQMinus<0.0) {oneSigPQMinus = 0.0;}
          if (oneSigPQMinus>1.0) {oneSigPQMinus = 1.0;}
          int PQMinus = 256 +(int)(3583.0*PQ10000_r( oneSigPQMinus)+0.5);
          
//          if (j == 0) { average_col[c][j] = MAX(1e-16, average_col[c][j]); /* prevent divide by zero */ }
          if (c==0) { printf(" sigma_red[%d] = %e self_relative = %f (%f%%) at average value = %e for %.0f pixels, PQ Ave = %d [%d, %d]\n",
            j, sigma_col[c][j], sigma_col[c][j] / average_col[c][j], 100.0 * sigma_col[c][j] / average_col[c][j], average_col[c][j], count_col[c][j], PQ, PQMinus, PQPlus); }
          if (c==1) { printf(" sigma_grn[%d] = %e self_relative = %f (%f%%) at average value = %e for %.0f pixels, PQ Ave = %d [%d, %d]\n",
            j, sigma_col[c][j], sigma_col[c][j] / average_col[c][j], 100.0 * sigma_col[c][j] / average_col[c][j], average_col[c][j], count_col[c][j], PQ, PQMinus, PQPlus); }
          if (c==2) { printf(" sigma_blu[%d] = %e self_relative = %f (%f%%) at average value = %e for %.0f pixels, PQ Ave = %d [%d, %d]\n",
            j, sigma_col[c][j], sigma_col[c][j] / average_col[c][j], 100.0 * sigma_col[c][j] / average_col[c][j], average_col[c][j], count_col[c][j], PQ, PQMinus, PQPlus); }
        } /* count_col[c][j] > 0 */
      } /* j */
    } /* c */
